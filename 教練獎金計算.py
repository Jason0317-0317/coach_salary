...
def generate_perfect_salary_report(uploaded_files):
    ...
    # 為了避免 pandas 欄位名稱重複報錯，巨蛋館的教練和課程欄位名稱尾部加上一個空白鍵
    cols_left = ["教練", "課程", "單價", "義昌館堂數", "義昌館金額", "高美館堂數", "高美館金額", "中山館堂數", "中山館金額", "三館總堂數", "三館總金額", "應付金額(三館)", "執行業務(扣款)_三", "補充保費(扣款)_三", "應付薪資(三館)"]
    cols_right = ["教練 ", "課程 ", "巨蛋館堂數", "巨蛋館金額", "應付金額(巨蛋)", "執行業務(扣款)_蛋", "補充保費(扣款)_蛋", "應付薪資(巨蛋)", "年資"]
    ...
    for coach in COACH_PRICING.keys():
        ...
        base_pay_3 = c_df["三館總金額"].sum()
        tax_3 = int(base_pay_3 * 0.1) if base_pay_3 >= 20000 else 0
        health_3 = int(base_pay_3 * 0.0211) if base_pay_3 >= 20000 else 0
        final_pay_3 = base_pay_3 - tax_3 - health_3

        # --- 2. 計算右側（巨蛋） ---
        base_pay_e = c_df["巨蛋館金額"].sum()
        tax_e = int(base_pay_e * 0.1) if base_pay_e >= 20000 else 0
        health_e = int(base_pay_e * 0.0211) if base_pay_e >= 20000 else 0
        final_pay_e = base_pay_e - tax_e - health_e

        # --- 3. 準備左右分割並合併 ---
        df_l = c_df[["教練", "課程", "單價", "義昌館堂數", "義昌館金額", "高美館堂數", "高美館金額", "中山館堂數", "中山館金額", "三館總堂數", "三館總金額"]].reset_index(drop=True)
        df_l["應付金額(三館)"] = df_l["三館總金額"]
        df_l["執行業務(扣款)_三"] = 0
        df_l["補充保費(扣款)_三"] = 0
        df_l["應付薪資(三館)"] = df_l["應付金額(三館)"] # 明細列不扣稅，小計才扣

        df_r = c_df[["教練", "課程", "巨蛋館堂數", "巨蛋館金額"]].reset_index(drop=True)
        df_r.rename(columns={"教練": "教練 ", "課程": "課程 "}, inplace=True) # 防止名稱衝突
        df_r["應付金額(巨蛋)"] = df_r["巨蛋館金額"]
        df_r["執行業務(扣款)_蛋"] = 0
        df_r["補充保費(扣款)_蛋"] = 0
        df_r["應付薪資(巨蛋)"] = df_r["應付金額(巨蛋)"]
        df_r["年資"] = seniority_val

        # 橫向合併
        c_combined = pd.concat([df_l, df_r], axis=1)
        final_dfs.append(c_combined)

        # --- 4. 製作「小計」列 ---
        sub_row = {col: "" for col in cols_left + cols_right}
        sub_row["教練"], sub_row["課程"] = coach, "小計"
        sub_row["教練 "], sub_row["課程 "] = coach, "小計"
        
        # 左側小計值
        sub_row["三館總金額"], sub_row["應付金額(三館)"] = base_pay_3, base_pay_3
        sub_row["執行業務(扣款)_三"], sub_row["補充保費(扣款)_三"], sub_row["應付薪資(三館)"] = tax_3, health_3, final_pay_3
        
        # 右側小計值
        sub_row["巨蛋館金額"], sub_row["應付金額(巨蛋)"] = base_pay_e, base_pay_e
        sub_row["執行業務(扣款)_蛋"], sub_row["補充保費(扣款)_蛋"], sub_row["應付薪資(巨蛋)"] = tax_e, health_e, final_pay_e
        
        sub_row["年資"] = seniority_val
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # 自動尋找需要合併儲存格的欄位索引 (包含左右兩邊的姓名與扣除款)
        merge_col_indices = []
        for idx, cell in enumerate(ws[1], 1):
            ...
            # 抓取要被合併的欄位 (這裡會抓到"教練"、"教練 "、稅款、年資)
            if str(cell.value).strip() in ["教練", "執行業務(扣款)_三", "補充保費(扣款)_三", "應付薪資(三館)", "執行業務(扣款)_蛋", "補充保費(扣款)_蛋", "應付薪資(巨蛋)", "年資"]:
        ...
        for r in range(2, ws.max_row + 1):
            ...
            for c in range(1, ws.max_column + 1):
                ...
                # 判斷是否為小計列 (檢查左或右的課程欄位)
                if str(ws.cell(row=r, column=2).value).strip() == "小計":
            ...
            if next_coach != coach_val:
                for c_idx in merge_col_indices:
                    ws.merge_cells(start_row=start_r, start_column=c_idx, end_row=r, end_column=c_idx)
                start_r = r + 1
